...
During imaging the user will supply a set of facet centre coordinates or number of facets splitting the sky (or both), along with a measurements database and image dimensions. The imager outputs a 
...
will contain $N_\text{baseline}\times \frac{\tau_\text{observed time}}{\tau_\text{integration length}}$ rows, including those contributed by auto-correlated antennas. It is worth noting 
...
This is less of a problem when performing faceting and w-projection, but can't be ignored. In order to mitigate this latency we opt to split the data into many pieces, making it possible
...
I/O is effectively hidden by this approach, and the runtime is bound by the compute time. All but the cost of loading the first chunk of data is hidden by this strategy.
