...
\section{Narrow field synthesis using the FFT}
 ...
 The direct inversion algorithm boils down to a summation of complex multiplications over the entire grid in the image space \cite[Lecture 7]{taylor1999synthesis}. This equation can be amended to be as accurate as needed, taking per pixel effects into 
 ...
 \begin{enumerate}
  \item The Fast Fourier Transform yields a \emph{tangent planar approximation} to the sky dome: the synthesized image is only accurate near the phase centre (by convention the center) of the image.
  ...
  \item The regularly sampled FFT assumes the signal (in this context the signal is the sky) repeats: sources outside the field of view of the produced image is flipped back into the image
	on the opposite side. This introduces the necessity to filter the image with a filter that only passes signal that is in the field of view being considered and stops
	any energy outside the field of view.
 \end{enumerate}
 ...
 account in the synthesis step.
 
 Our approach focuses on a variant of the relatively fast class of linear interpolation operations:
 \begin{equation}
 ...
 guasian, B-spline, etc. and may additionally be windowed by one of the large number of window functions as in the case of the sinc function, 
 for example Hamming, Hanning, Blackman, Kaiser-Bessel, along with many others.
 
 One of the primary constraints are that the interpolation operator must be very fast. 
