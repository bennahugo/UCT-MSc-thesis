...
In addition to the $\psf$ deconvolution problem is the telescope calibration problem. Appart from removing unwanted
...
\begin{figure}[h]
 ...
 \begin{tikzpicture}[node distance=4.5cm]
  ...
  \node (stop) [startstop, below of=finalize] {};
...
The self-calibration process relies on a knowing some aspects of the sky and can include one or more well-described sources. It includes the 
following steps \cite{2011A&A...527A.107S}:
 \begin{enumerate}
  \item Gain minimization per antenna, and the minimization of the residual amplitudes and phases associated to each individual baseline.
  \item Imaging, deconvolution and model updating using H\"ogbom CLEAN
 \end{enumerate}
 
In reality this simple gain-adjustment process cannot remove the polarization effects due to the primary beam, nor can it solve for unknown slow-varying 
directional dependent gain terms. If we for the moment only consider the known polarization effects of the antenna beam (which can be modeleled or 
measured using Holography) it becomes clear that some sources will be more resolved than others for the same amount of observation time, 
...
Since these effects vary with both direction and time, removing them is a tricky proposition; at every timestep since only a 
...
LOFAR array (refer to the Awimager implementation by Tasse et al. \cite{tasse2013applying}).

The ``union-form'' of the RIME stated before can be rewritten in terms of 16-element muler matricies where: 
\begin{equation}
 D_{pq}(l,m,\nu,t) := D_{p}(l,m,\nu,t) \otimes D_{q}(l,m,\nu,t) 
\end{equation}

To accomplish this invertion it is necessary to assume that the 16 terms of the Muler matrix $D_p(l,m)\otimes D_q(l,m)$ are relative

Solutions to direction independent gains (the G terms of the RIME) can be computed without image synthesis, but 
