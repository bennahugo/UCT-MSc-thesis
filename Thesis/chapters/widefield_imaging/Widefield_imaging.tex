...
The RIME can be thought of as predicting what measurements the telescope should make, given a model radio sky (\emph{intensity distribution}), telescope behavior and environmental effects as inputs. 
...
it is necessary to divide through by the number of samples in the neighborhood of each point in the measurement space, \emph{uniformly} weighting the synthesized image. The tracks also highlight an important aspect about
...
effects in wide-field imaging is the antenna primary beam. We will return
...
\section{Narrow field synthesis using the FFT}
 ...
 this filter in their implementation of a w-stacking (explained in the next section) imager to great success. The Keiser-Bessel window is defined as:
 \begin{equation}
...
Rick Perley \cite[Lecture 19]{taylor1999synthesis}\footnote{A word of thanks to Richard Perley for his insightful discussions on the problem and clarification on his 
tangent facet imaging approach in late 2014}, Tim Cornwell and Rick Perley \cite{cornwell1992radio}, Cornwell, Golap and Bhatnagar \cite{cornwell2008noncoplanar}, Kogan and Greisen \cite{aipsfaceting}, and 
...
As Richard Perley \cite[Lecture 19]{taylor1999synthesis} points out this phase difference is not a physical delay in the strictest sense of the word, but arise merely because of the 
geometry of the array and coordinate systems. In essence it arises because the field of view is too wide, or it is sampled with a tilted interferometer or both problems arise simultaneously. The effects 
...
To correct this effect one may consider employing a three dimensional Fourier transform. Richard Perley \cite[Lecture 19]{taylor1999synthesis} shows that the 
...
transform to overcome the aliasing effects introduced by the longest baseline. Instead Richard Perley \cite[Lecture 19]{taylor1999synthesis} mentions 2 more 
...
The goal in faceting is to approximate a wider field of view by many small narrow field images. Richard Perley and Tim Cornwell \cite{cornwell1992radio} propose a 
...
Richard Perley \cite[Lecture 19]{taylor1999synthesis} shows that only employing a phase shift per visibility without regard of the tangency of the resulting facets will require the creation of many more facets 
...
Another important remark is that the resolution of each facet image is not arbitrary, but is subject to the same Nyquist sampling 
...
polyhedron-faceting algorithm. Bill Cotton\footnote{A word of thanks goes to Bill Cotton for useful discussions on facet imaging in late 2014} 
