...
All of the approaches above attempt to either drive $w$ down to zero (snapshots or w-projection) or drive $(n-1)$ down to zero (facet imaging). There is no reason why
...
\begin{algorithm}
  ...
  \FORALL {facets centres $(\alpha_i,\delta_i)$}
    \FORALL {channels $c$ in the continuum image band}
      ...
      \STATE {Let vis' = vis * $p_f$, by element-wise phase shift}
...
For our work we assume the projection coordinates are the orthogonal coordinates specified here. It is well worth noting 
...
One strategy to include $w\epsilon$ per gridded visibility is to Taylor expand the term to a first order approximation around the original phase centre. The 
...
As already mentioned the core idea in w-projection is to multiply the intensity distribution a w-dependent phase screen. Cornwell, Golap and Bhatnagar \cite{cornwell2008noncoplanar}
...
\begin{equation}
 \begin{split}
        V(u,v,w)&\approx\int{\int{I(l,m)e^{-2{\pi}i[u(l-l_i)+v(m-m_i)]}e^{-2{\pi}i[w(n-n_i)]}\frac{dldm}{n}}}\\
		&\approx\int{\int{I(l,m)e^{-2{\pi}i[u(l-l_i)+v(m-m_i)]}\mathfrak{w}_w(l,m)\frac{dldm}{n}}}\\
		&\approx V(u,v,w=0)*\mathfrak{W}_w(u,v)\\
	V(u,v) * \mathfrak{W}_w(u,v) &\rightleftharpoons I(l,m)\mathfrak{w}_w(l,m)\\
 \end{split}
\end{equation}
...
field imaging, it is still important to include the response of the anti-aliasing function. There is obvious closed-form expression for the 
...
The immediate concern is determining how many $w$-planes must be precomputed. Here it is useful to note that the maximum difference in the $w(n-1)$ during the
...
$N_{\text{planes}}$ different $w$-planes (here assumed to be linearly separated) between the $w_{\text{max}}$ and the $0^{th}$ plane, such that the difference 
between planes is a fraction of the image cell size:
\begin{equation}
...
The next question that arise is why it may be useful to combine faceting and w-projection if w-projection can remove the widefield effects altogether. The answer
...
\begin{figure}[h]
  \begin{mdframed}
    ...
    \caption[w fringe in one dimension]{Here the real part of the w phase term is plotted. It is clearly visible that the fringe is not scaled evenly over the field of view.
	       ...
	       satisfies the Nyquest sampling criteria. As expected for larger values of w (longer baselines) the frequency of the fringe oscilations is
	       also increased.}
    \label{fig_w_fringes}
  \end{mdframed}
\end{figure}

By decreasing the field of view of each of the synthesized facets, it is ensured that the phase screen do not ossilate nearly as frequently as with a regular w-projection approach,
and decreases the required filter support size.
