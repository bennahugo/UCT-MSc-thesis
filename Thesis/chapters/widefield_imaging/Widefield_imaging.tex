...
\section{Narrow field synthesis using the FFT}
 ...
 higher resolution image. It is quite important to realize however that in the gridding (and degridding) steps the data is simply smeared out into the continuous
 measurement space and by implication also over the newly inserted zeros. To help visualize this ``convolutional gridding'' process refer 
 to Figure~\ref{fig_gridding}.
 \begin{figure}[h]
  \begin{mdframed}
 ...
 define another criterion here. Sze Tan \cite{tan1986aperture} for instance defined this as minimizing the difference between the Direct Transform and the 
 FFT approach}:
 \begin{equation}
 ...
 truncation of the shah and convolving functions also contribute to the aliasing.
 ...
 \begin{figure}[h]
  \begin{mdframed}
    ...
    \end{subfigure}
    \caption[Alias reduction]{Here a ``grid'' sky model was simulated using MeqTrees and imaged with our imager, first using Nearest Neighbour
    and then using an ordinary unwindowed $\sinc$ function. Sources that fall slightly outside the field of view are aliased back in when the energy
    outside the field of view is not limited as expected.}
 ...
 astronomical imagers and a more detailed analysis is given by \cite{schwab1984optimal}. The computation of these family of 
 spheriodal functions (as defined by Rhodes \cite{rhodes1970spheroidal}) maximizes the energy concentration ratio in 
 Equation~\ref{eqn_aliasing_energy} much better than many other functions considered by Schwab in \cite{schwab1984optimal}:
 \begin{equation}
  |1-(2x/W)^2|^\alpha\psi_{\alpha0}(0.5\pi W,2x/W
 \end{equation}
 
 Here $W$ is again the full support of the the convolution filter and $x$ increases in steps of $\Delta{u}$. If $\alpha=0$ the function
 as it stands is called the \emph{prolate spheriodal function}. When $\alpha>0$ a weighted energy concentration ratio is maximized instead.
 
 To conclude our comparison we give some extracts from the literature in Figure~\ref{}
