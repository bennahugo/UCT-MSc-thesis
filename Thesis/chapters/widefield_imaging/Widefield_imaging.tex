...
\section{The self-calibration, imaging and deconvolution pipeline}
...
observed visibilities. In fact this major-minor cycle approach ideal when dealing with multiple adjacent fields. 
...
In addition to the $\psf$ deconvolution problem is the telescope calibration problem. One of the more pronounced directional 
dependent effects is the antenna primary beam in wide field imaging. It is possible to decern bright sources in the sidelobes 
of the antenna beam (and they will contribute to the $\psf$ convolution just the same as sources inside the primary beam). However, 
as already mentioned, the antenna gain vary accross the field of view. This means that some sources
will be more resolved than others for the same amount of observation time, depending on their position in the sky. 
Adding to the complexity, the beam pattern can be anisotropic; one cause can be the struts above a prime-focus antenna. 
If the antennae are placed on alt-azumuth mounts the sky rotates with respect to this anisotropic beam pattern over 
the course of an observation. This results in sources that is only partially resolved everywhere other than well 
within the primary beam. The correction of this effect alone is not a trivial undertaking and involves a convolution
