...
approximation ($\sqrt{1+x}\approx1+\frac{x}{2}$) to $\mathfrak{w}_w(l,m)$ with respect to a facet centre is used, the following linearly separable relation is obtained:
\begin{equation}
   \mathfrak{w}_w(l,m) = e^{2{\pi}iw[(l_i^2-l^2)/2]}e^{2{\pi}iw[(m_i^2-m^2)/2]}
\end{equation}

Separating out $l_i$ and $m_i$ to be applied by the normal faceting phase shift we get the same small angle version of the equation. Assuming the 
...
$N_{\text{planes}}\times[W_{sup} + (W_{sup} - 1)\times(m_{\text{oversampled}} - 1)]\mathbb{C}$. This is 
