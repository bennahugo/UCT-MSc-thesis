...
quite empty if this was the only source of radio emission in nature and it placed a dampner on radio observation until 
Karl Jansky's observation of radio electromagnetic radiation stemming from cosmic sources in the early 1930's sparked interest in observing 
...
For the sake of discussion and to simplify the mathematics later on we can make some simplifying assumptions about here:
\begin{enumerate}
...
Since those early days significant progress has been made towards increasing in the sensitivity and size of these radio telescopes. Next we will explore 
...
either a narrow or a wide band of frequencies. 
...
For astronomical purposes it is \textit{very important} to have an antenna much, much larger than $\lambda$ in order to increase both sensitivity 
...
the resolution of finer details depend squarely on the angular resolution of the observing telescope:
...
Using this total power relation it is easy to see the advantage of averaging several bands together to observe sources of continuum emission, although
observation of spectral lines (such as the absorbtion line of abundant n
...
If one of the sensors is moved very slightly further away from the disturbance the delay between the measurement taken by
...
towards the same sky position. Just as with the bowl of water analogy if the source is slightly away from the pointing centre of the
telescope, the phase delay between the arival of the wave front at two seperate antennae will tell us something about its 
...
those from directions $\theta = (n + \frac{1}{2})\pi$ will be invisible to the interferometer.
...
this perfection is never attained. The sensitivity of the complex interferometer depends firmly on the total collecting area 
as we already discussed, the resistivity (and associated temparature) of the equipment, including the antenna and amplifying 
...
At a physical level, sources do vary slightly over time, although we expect their average intensity is centered around 
...
of tracking steller sources.



Before moving on to a more mathematical model of an interferometer it is necessary to amend the coordinate system 
defined earlier for a single antenna telescope to be relative to some fixed location (it can be any) and the 
antennae in an array will be measured relative to this new coordinate frame. Assume a single antenna is 
picked as reference point and all the other antennae positions are measured relative to this ``reference 
antenna''. 
