...
\begin{itemize}
 \item \textit{Antennas: Fundamentals, Design, Measurement} \cite{blake2009antennas} serves as a good introductionary text on general [communications] radio antennae design from an electrical engineering perspective. Chapters 1 through 4
 are very insightful.
 ...
 \item \textit{Radiotelescopes} \cite{christiansenradiotelescopes} gives insight into the historic development of radiotelescopes from the 1930s through the 1960s, with a focus on telescope design, interferometry and a 
 good introduction to the field of radio astronomy. The book is in the public domain and freely available from \url{https://archive.org/details/Radiotelescopes}.
...
of light. Recall from elementary physics that the frequency and wavelength of a wave is related:
\begin{equation*}
 \nu = \frac{v}{\lambda}
\end{equation*}
where:\\
...
In a vacuum, far away from obstacles (\textit{free space}) electromagnetic waves propagate at a constant velocity, $c\approx 3\text{x}10^8$. This velocity is only slightly reduced
when propagating though most other media.

We can conveniently measure these waves with telescopes of various form at either ground-level or from planetary orbit. This electromagnetic energy can be emitted by both thermal and non-thermal sources. 
The intensity of the radiation emitted from thermal sources is determined by both the temperature of the source and the measured frequency. On the other hand a good example of non-thermal emission is the 
synchrotron emission by relativistic electrons accelerated through magnetic fields. A significant portion of radio astronomy focus on the non-thermal sources. These are generally very intense sources of 
electromagnetic radiation and are very far away. We can make the simplifying assumption here that radio sources emit this radiation outward uniformly in all directions (they are \textit{isotropic}) 
...
propagates forward radially and a new wavefront is formed where they experience maximum constructive interference. 
...
When these waves propagate through some medium other that a vacuum this decrease in directional energy is not the only form of attenuation. In the 
...
In addition to the optical properties of electromagnetic waves and their attenuation one has to consider the direction of propagation of each point on the incoming wavefront. If most of the energy of each wave is strongly 
directional the wave is said to be \textit{polarized}. For polarized emissions the path traced by a single point of either electrical or magnetic component will be highly regular. It will 
remain in a single plane (\textit{linear} polarization), will spiral at a fixed radius (\text{circular} polarization) or will spiral eliptically. Circular polarized waves can be thought 
of as the combination of two perpendicular linearly polarized waves of equal strength (if the two wavefronts are not equally strong the resultant polarization will be eliptical). We can draw on an 
application of this property from an everyday context: in the visible spectrum sunglasses will filter out all light except vertically polarized light, in order to reduce glare. A single-feed radio 
antenna will similarly measure a single directional component, and will therefore only useful in measuring strongly polarized sources. Such antennae are not useful for general observations, save for 
those of pulsars. In most contexts it is more useful to have two perpendicular feeds (a dipole comes to mind) in order to fully describe the incoming wavefront (or at least the electric component thereof)
of the field parallel to the dipole. When the energy measured by both feeds are roughly equal the emitting source is said to 
be \textit{unpolarized}.
...
moving onto the topic of aperture synthesis with array-based telescopes.
...
To simplify our discussion we'll only consider directional antenna (a simple parabolic reflector with an axial feed above the center of the parabola is 
...
(where the waves are considered as rays) used in optical telescopes are of very limited use. It is much more preferable to describe these antennae in terms of 
physical optics.
...
energy at one single point, as one would expect when using geometric optics, an interference pattern is formed, where
...
constructive interference will be noticeable, along with several minor lobes to either side of that primary beam (the lobes right 
...
lobe, but may have many lobes of equal amplitude. 
...
\begin{figure}[ht]
 \begin{mdframed}
  ...
  \caption[Collection of electromagnetic wave energy and response]{Here the diffraction pattern of a parabolic aperture is 
  illustrated with a single cutting plane. This is analogous to the diffraction pattern of visible light when passing through 
  a single narrow slit. Directional antennae form a discernible primary lobe, with several minor lobes of decreasing magnitudes to
  either side. The beam width of the primary lobe is [usually] defined as at the level where the electric intensity is halved (around $1/\sqrt{2}$).
  At approximately $\frac{\lambda}{D}$ the ``first null'' appears ($D$ is the diameter of the aperture). This is the first point of complete destructive interference.
  Images taken from \textit{Radiotelescopes} \cite{christiansenradiotelescopes} and \textit{Synthesis Imaging II} \cite{taylor1999synthesis}
...
the effective area of the antenna. This is defined in terms of the gain of the antenna, as:
\begin{equation*}
 A_e = \frac{G\lambda^2}{4\pi}
\end{equation*}
Assuming absolutely no losses in the measured electric density this effective area will approach the geometric area of telescope in the direction
...
either a narrow or a wide band frequencies. Note, however, that no real antenna system will cover a perfectly-rectangular band 
...
The variation of intensity over the emitting source and its variation over a band of frequencies are also of
significant physical importance. Generally speaking, black-bodies (sources that near-perfectly absorb all incoming electromagnetic radiation)
...
this type. The intensity distribution of black-body emission depends on both the equilibrium temperature of the body and the frequency of emission as we alluded 
...
\begin{figure}[ht]
 \begin{mdframed}
 \centering
 \includegraphics[width=0.5\textwidth]{images/plank_dist.png}
...
This is by no means the only requirement from an astronomical perspective. In order to track the evolution of the universe astronomers may track certain
very common elements such as neutral hydrogen. To accomplish this they observe very narrow bands, discretized into many, densely spaced 
channels (essentially forming a spectrometer). Since these common elements absorb electromagnetic radiation at known frequencies ($\lambda = 21 cm$ for neutral hydrogen and
$\lambda \approx 18 cm$ for hydroxyl). Detecting and tracking these common elements are of importance in measuring the radial velocities through their \textit{Doppler frequency shift}.
Any city-dweller experience this effect on a daily basis: the siren of an approaching emergency services vehicle will sound more high pitched as that vehicle
approaches and will decrease in pitch as soon as the vehicle has passed. Similarly these absorption lines will appear to shift to a higher
frequency if those elements move toward the observer at velocity $v$ (a ``blue shift'' at the wavelength of visible light). When moving 
away from the observer the absorption line will appear to shift to a lower frequency (a ``red shift'' at the wavelength of visible light). These 
apparent shifts can be used to track, for example, the angular velocity of galaxies: detected hydrogen in one side of the galaxy will blue shift as it moves 
towards the observer, while the other side will red shift as it moves away from the observer.
...
collected planar wavefront. This is the electrical flux density measured in units $S = Wm^{-2}Hz^{-1}$. It is useful to remind the reader that power is proportional to the square voltage and may 
...
measured in wavelengths (and is therefore measured in meters). The direction of the a point on the sky (a unit sphere) are given by the 
direction cosines with regard to the local frame, denoted by l, m and n respectively (note that $l^2 + m^2 + n^2 = 1$). 
Both $l$ and $m$ may range between $\pm\pi \text{ radians}$ (from horizon to horizon in both directions).

Now let $\Omega$ be a \textit{small} solid angle subtended by a very small area. This solid angle is measured in square radians (in steradians or ``sr''). A visualization of the measured flux density
...
per steradian over the entire solid angle per unit frequency must then be 
  \begin{equation}
    p = \int_{\Omega}{A(l,m).I(l,m)\cos{\theta}d\Omega}
  \end{equation}
where\\
...
downplayed (somewhat ignorantly) the effects of the atmosphere and the radiativity of the antenna (the antenna has an internal temperature which means 
it radiates heat, and may be thought of as a resistor). We will come back this point when discussing calibration.
