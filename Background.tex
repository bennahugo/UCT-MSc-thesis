...
all contributing sources. It was assumed that these radiate independently of each other. The effects of the atmosphere and the radiativity of the antenna (the 
antenna radiates because it has an internal temperature, and may be thought of as a resistor) were also downplayed. We will come back this point when discussing calibration. 
In reality a small band of frequencies are measured which results in smeering. A more thorough definition of measurement for array based telescopes will be given, since
those are the ones of primary interrest in this thesis. 
...
Measuring this interference pattern is not ideal to resolve large structures. One way around this problem is to take an additional 
...
produce a response pattern that is sensitive over a wider range of angles. From this point forward whenever we refer to correlator 
we will be referring to this \textit{complex} correlator. It is convenient to think of the measurements made by such a complex 
...
but will be that of an complex measurement, instead of only dealing with the real domain.
...
The coordinate system defined for a single antenna coordinate system will have to be amended slightly to be relative to some fixed
location (it can be any) and the antennae in an array telescope will be measured relative to this new coordinate frame. Lets assume
...
of the $0^h$ hour hand, at a declination of 0, Y lies on the same plane as X, but points to $18^h$ and Z points straight up 
...
the equatorial and alt-azimuth coordinate systems.
\begin{equation}
...
Ignoring the effects of polarization, the directional antenna gain (the beam, as before) and any atmospheric delays and attenuation
for a moment, the correlator will measure values should be proportional to (stemming directly from equation~\ref{REF_EQN_SINGLE_ANTENNA}):
\begin{equation*}
  \begin{split}
...
Notice that here the expression for p is relative and depends only on the directions of each contributing source. Of course this
is all relative to the pointing centre of the telescope. We multiply through by a complex exponential. This adds the appropriate 
delay to ensure measurements are taken relative to the pointing center. It may help to think of this as measuring the
incoming [complex] flux from the direction $\theta$ shown in Figure~\ref{fig_interferometer}:
\begin{equation}
...
\textit{always} measured in terms of wavelength (and by relation frequency) as evident in equation~\ref{EQN_RIGHTHANDED_SYSTEM}.
...
Figure~\ref{fig_interferometer} $\tau$ depends on the orientation of the baseline with respect to source and the length of the 
...
Up to this point we conveniently only discussed correlator output for a single feed of each antenna in a two element interferometer.
Just as in the case of a single element telescope the response from such a correlator will be sensitive to highly directional sources.
The correlator may therefore additionally correlate all four combinations between the two polarization feeds of each antenna to form 
a 2x2 matrix of short time averages. Without any loss the flux density average ($B$) in equation~\ref{EQN_ONE_CORR_RIME} can be
changed to include all four averages. The exponential term then becomes a scalar 2x2 matrix.

The four Stokes paramters are an easy way to describe the polarization of the measured electromagnetic field. These 
are non-physical quantities, but give an easy way to express the degree to which the field is linearly or circularly polarized
(and anything inbetween). Each of the parameters Q, U and V are linearly independent and discribe a position on the 
Poincar\'e Sphere (figure \ref{fig_poincare}). I is the total measured intensity. The coordinates of Q, U and V on 
those given by:
\begin{equation}
\begin{split}
...
Equation~\ref{EQN_ONE_CORR_RIME} may thus be rewritten to take multiple polarizations, equipment gains and directional effects into account through a \textit{Jones} matricies 
formulation presented by Smirnov in a series of papers on this formulation and its implications for calibration \cite{2011A&A...527A.106S,2011A&A...527A.107S,2011A&A...527A.108S}:
...
written as two-dimensional matricies. Each antenna will additionally have its own set of these effects. They 
include corrections for Faraday rotation, ionospheric effects, tropospheric effects, the primary beam (and any rotation over 
long observations) and pointing error due to weather (wind and heat). These corrections only account for amplitude scaling and 
...
may not necessarily commute, so the order of multiplication is important). The different effects will be described later 
when calibration is discussed.
...
dishes and reduce afterwards. This means that the data rates produced in correlation grows quadratically with 
number of antennae (the number of possible baselines). This includes the auto-correlated baselines (correlation 
of each antenna with itself).
\begin{equation*}
  \text{number of baselines } = \frac{n(n-1)}{2} + n
\end{equation*}

Inquisiting readers may, rightfully, argue that even if many baselines are used the synthesized aperture will be
very sparsely sampled, especially when discussing very large arrays. In fact only a handful of small points will be 
...
The second, more promising way of achieving this with much larger arrays is to use the rotation of the earth to rotate
the baselines radially with respect to the rotation axis of the planet. This may sound like a radical concept, but it 
follows naturally from the way we defined the local coordinate frame in equation~\ref{EQN_RIGHTHANDED_SYSTEM}. If the
array tracks some point on the sky at some fixed declination over an extended period of time, the change in the hour
angle will rotate every baseline. This become clearer when recasting the equation from parametric to implicit form. 

Start by obtaining the individual expressions for u,v,w in equation~\ref{EQN_RIGHTHANDED_SYSTEM}. If the expressions
for u and v are manipulated into the familiar form of an elipse it is easy to show (although tedious) that the following
relation holds:
\begin{equation*}
 u^2 + \left(\frac{v-(L_Z/\lambda)\cos{\delta_0}}{\sin{\delta_0}}\right)^2 = \frac{L_X^2 + L_Y^2}{\lambda^2}
\end{equation*}

This elipse along the v axis by $\frac{L_Z\cos{\delta_0}}{\lambda}$. The major axis has a length of 
$\frac{\sqrt{L_X^2 + L_Y^2}}{\lambda}$. It also follows that at low declinations the baselines 
of a pure east-to-west array will be nearly parallel to the pointing direction of the telsecope.
Not only will the for-most antenna shadow the antennae behind it, but there will be little coverage of the u, v
frame.

To make this more concrete consider a ficticious observation with the Extended Very Large Array. The antenna 
positions for all 27 antennae, baselines and projected tracks are shown in Figure~\ref{fig_evla_observation}. With
...
images are synthesized. We will come back to this later on.
