...
Since Martin Ryle and his associates made their revolutionary observations using this approach there has been
...
This is appropriately called \textit{Very Long Baseline Interferometry}. A discuss on what is meant by an interferometer 
and why array telescopes are referred to as interferometers will follow very shortly (although it may be easy to guess given
the discussion on single antenna telescopes). Timing and data transport issues then become a major obstacle, and the data is usually 
physically shipped to a central location. Combination over the Internet (e-VLBI) has only recently become feasible. Keen readers may 
...
be the same - since the waves propagate outward at the same speed and with the same amplitude. 
...
directions $L.\cos{\alpha} = n + \frac{1}{2}$ will be out of phase ($n\in\mathbb{Z}$). The delay between the arrival
times is given by $\tau=\frac{\vec{b}\cdot\hat{s}}{c}$.
...
is formed on a screen when light passes through two small slits separated by a distance $L$ in front of the screen. Here 
the size of the slits are small in comparison to $L$ and the first null point occur at a distance proportional to 
$\frac{\lambda}{L}$. from The only major difference here is that the slits are in fact the antennae.
