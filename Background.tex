...
\subsubsection{Self-calibration}
 ...
 evaluation over all the observed visibilities (M of them in total). This approach requires on the order of $O(N^2M) \approx O(N^4)$ sine and cosine evaluations (where N is the size of a single dimension of a square grid) for a large number of visibilities. This approach
 has the advantage of being highly parallel requiring no atomic operations. Atomic operations are generally required whenever two parallel processes attempt to update the same memory location simultaniously, and this update operation requires separate memory reads and writes. This ensures
 that the changes made by the first process is not overridden by the second. This algorithm is simply stated as a summation of complex multiplications over the entire grid in the image space \cite[Lecture 7]{taylor1999synthesis}:
 \begin{equation}
 ...
 \cite[Lecture 7]{taylor1999synthesis} to prolate spheriodal functions and approximations to these functions, such as the Keiser Bessel function \cite{jackson1991selection}. Sze Tan \cite{tan1986aperture} describes derives functions that out-perform prolate spheriodal functions
