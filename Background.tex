...
\end{figure}
 ...
 \textit{Direct Fourier Transform} approach (explained in the next section) is computationally prohibative, when compared to the convolution gridding approaches, coupled with either 
 W-projection-based or faceting techniques, although it has the advantage of being much more accurate \cite{2011A&A...527A.107S}.
...
\subsubsection{Self-calibration}
 ...
 peeling and the method of differential gains. Peeling is widely tested and involves iteratively removing the effects from the brightest sources, while differential gains simultaniously solves for the gain effects from bright and faint sources using a mix of
 Direct Fourier Transform and traditional Discrete Fourier Transform-based approaches \cite{2011A&A...527A.107S,2011A&A...527A.108S}.
 
 \section{Imaging through convolutional gridding}
 There is generally two techniques used to transforming between the observed visibilities and the (dirty) image. Either a brute force Direct Fourier Transform is taken where each grid point is approximated through a brute-force
 evaluation over all the observed visibilities (M of them in total). This approach requires on the order of $O(N^2M) \approx O(N^4)$ sine and cosine evaluations (where N is the size of a single dimension of a square grid) for a large number of visibilities \cite[Lecture 7]{taylor1999synthesis}. The second approach is to 
 interpolate the data onto a regularly sampled grid, by filtering with a two-dimensional convolution function, in order to use the Fast Fourier Transform to approximate the results obtained through taking a Direct Fourier Transform. This Fast Fourier Transform has a computational complexity of $O(N^2\log_2{N})$.
 Two dimensional filtering typically requires $O(MC^2)$ operations (where C is the size of each dimension of a square filter). For very large filters this operation is prohibatively expensive. However, by using linearly seperable filters
 as mentioned previously the complexity of the operation is reduced to $O(MC) \approx O(N^3)$ for a filter with its number of taps roughly equal to $N$ and visibilities covering most of the u,v space \cite{smith1997scientist}. If both the image size is small and the number of visibilities 
 ...
 considering the sheer number of baselines involved.
 ...
 gridding accounts for nearly 35.4\% of the computation time, while the inverse process of degridding accounts for 48.74\%. In this chapter we will primarily focus on the convolutional gridding algorithm, optimal
 ...
 \begin{tikzpicture}[font=\tiny]
    ...
    \draw[->] (4,0.5) -- (4.5,0.5);
    \draw (4.5,0) rectangle (6.5,1) node at (5.5,0.6) {Gain \&} node at (5.5,0.4) {DDE correction};
    \draw[->] (6.5,0.5) -- (7,0.5);
 ...
 We presume that the calibration terms will include corrections for Directionally Dependent Effects (per frequency, time step and antenna). An Inverse Fast Fourier Transform is taken after
 the observed visibilities are uniformly gridded. This dirty sampled image is subsequently deconvolved and the model image updated as per description of the CLEAN algorithm in the previous section.
 After this an estimate of the CLEANed visibilities are obtained through an FFT and degridding process. Normally several iterations of this expensive self-calibration cycle is required to converge to
 a final image of reasonable quality.}
 \label{fig_image_cycle}
 \end{mdframed}
 \end{figure}
 
 Exploiting the computational efficiency of Cooley-Tukey Fast Fourier Transform and its inverse to compute the forward and inverse Discrete Fourier Transforms requires, amongst other considerations, that
 ...
 \begin{figure}
  \begin{mdframed}
   ...
   \caption[Illustration of convolutional gridding]{Each observed visibility (per frequency channel and baseline) is centered at a precomputed coordinate in u,v space and is convolved with 
    ...
    up to 100 times more regularly than the grid itself \cite{varbanescu2008performance}. It should be clear that this is no ordinary convolution as discussed earlier. The fact that the u,v coordinates may vary 
 ...
 the effects of the Point Spread Function through deconvolving with the CLEAN algorithm, but deconvolution generally requires a sufficient number of samples to obtain a satisfactory inverse \cite{taylor1999synthesis}.
 
 This sampled observed visibility plane is then convolved with a densely sampled convolution function. The primary goal of this convolution is to limit the aliasing energy outside the support region of the convolving function, as well as 
 the effects of using gridding and the Fast Fourier Transform, instead of the Direct Fourier Transform. Depending on the metrics used to define ``optimal gridding function'' these may vary from a simple truncated sinc or exponential function \cite[Lecture 7]{taylor1999synthesis} to prolate spheriodal functions
 and approximations to these functions, such as the Keiser Bessel function \cite{jackson1991selection}.
